...
> For local development point an alias at the source venv so edits take
> effect without pipx reinstall:  
...
Textual dashboard inspired by `mo status`, augmented with project
attribution:

```
┌───────────────────────────────────────────────────────────────────┐
│ Health ●74 · MacBook Pro · M1 Max, 32GPU 8P+2E · 64G/1.8T · ...  │  ← dense single-line header
├───────────────┬───────────────────┤
│     CPU       │     Memory        │  P / E cores split; swap + pressure
├───────────────┼───────────────────┤
│   Thermal     │     Battery       │  cell temp · cycles · health · flow
├───────────────┼───────────────────┤
│  AI CLI       │  Top Projects RSS │  attributed by project root
├───────────────┴───────────────────┤
│        Listening Ports            │  port + pid + project + launcher
└───────────────────────────────────┘
```

**Keys**: `q` quit · `r` fast refresh · `R` slow refresh · `p` pause · `d`
dry-run · `k` kill selected · `K` force-kill · `1/2/3` focus AI /
Projects / Ports table · `+/-` adjust fast tick · `[/]` adjust slow tick.

## Roadmap

- [x] `cool status` — one-shot dashboard (+ Top Projects by RSS)
- [x] `cool watch` — full-screen Textual live dashboard + P/E cores + battery panel
