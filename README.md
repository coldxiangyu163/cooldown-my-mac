...
`cool` 是一个 macOS CLI，把每个进程归到「**项目 / AI CLI / 启动器**」三个维度，并提供批量回收、内存压力守护、24/7 launchd 托管。它**不替代** [Mole](https://github.com/tw93/Mole)（磁盘）/ [mactop](https://github.com/context-labs/mactop)（硬件读数）/ [stats](https://github.com/exelban/stats)（菜单栏），而是补上它们不管的那块——你的开发工作负载。
...
随便一台日常挂着几个 AI CLI 的 Mac 上跑 `cool dev`：
