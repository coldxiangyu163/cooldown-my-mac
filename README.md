...
**CPU runaway 可见**
- `Hot Processes by CPU%` 面板按**所属应用聚合**当前 CPU 占用（cores / %sys / 进程数），**不限 AI 家族**——MCP 子脚本、Chrome renderer、第三方 GUI 全都浮上来；agent-browser / puppeteer / playwright 泄漏的自动化浏览器会单独标 ⚠
...
- 自动清理 agent-browser / puppeteer / playwright 泄漏的孤儿浏览器（整棵 Chrome 子树；守护默认关闭，又老又静才收割，活跃会话豁免）
...
cool reap --leftovers --dry-run   # 预览泄漏的自动化浏览器残留（agent-browser/puppeteer/playwright），杀时连整棵 Chrome 子树
```
...
cool daemon config-init           # 写出 ~/.config/cooldown/daemon.yaml（含 leftovers 自动收割规则，默认关闭）
